"""
coadapt: Co-adaptation experiment combining network-based opinion dynamics
...
def coadapt_trial(G, N, R, delta, beta, pi, tau0, psi0, nu0, alpha, eps,
    ...
    :param G: a networkx graph (ignored if rule is 'none')
    ...
    """
    assert rule in ['b2sim', 'b2a', 'd2d', 'd2a', 'a2a', 'd2sim', 'd2max',
                     'b2b', 'none'], f'ERROR: Unrecognized rule "{rule}"'
    ...
    for r in range(R):
        # --- Authority RMHC: propose parameters ---
...
def coadapt_sweep(G, N, R, pi, alpha, eps, rule, w, sigma_tau, sigma_psi,
    ...
    resultsdir = osp.join('..', 'results',
                          f'coadapt_N{N}_R{R}_{pi}_{rule}_S{seed}')
...
def plot_trial(taus, psis, nus, pol_costs, pun_costs, alpha, pi, delta, beta,
               delta_hist, beta_hist, act_hist, rule, title=False):
    """
    Plot the evolution of authority costs & parameters and population desires,
    boldnesses, and actions in a single co-adaptation trial.
    ...
    :param delta_hist: an Nx(R+1) array of dissent desire histories
    :param beta_hist: an Nx(R+1) array of boldness histories
    :param act_hist: an NxR array of action histories
    :param rule: the string adaptation rule used
    :param title: True iff the plot should have a title detailing parameters
    """
    fig, ax = plt.subplots(4, 1, figsize=(5, 8), sharex=True, dpi=300,
                           layout='constrained')
    R = len(taus)
    ...
    if title:
        ax[0].set_title(r"Co-adapt ($\pi$ " f"={pi}, "
                        r"$\alpha$ " f"= {alpha}, rule={rule}) vs. Population "
                        r"$\delta_i \sim$" f"Exp({delta}), " r" $\beta_i \sim$"
                        f"Exp({beta}" r"$^{-1}$")

    # Plot authority parameters over time.
    ...
    ax[1].set(ylabel='Authority Parameters')

    # Plot mean desired dissent over time.
    ax[2].plot(np.arange(R + 1), delta_hist.mean(axis=0), c='darkorange',
               label=r'Mean $\delta$')
    ax[2].fill_between(np.arange(R + 1),
                       np.percentile(delta_hist, 10, axis=0),
                       np.percentile(delta_hist, 90, axis=0),
                       alpha=0.25, color='darkorange')
    ax[2].legend(loc='best', fontsize='small')
    ax[2].set(ylabel=r'Desired Dissent $\delta$')

    # Plot mean boldness and mean action over time.
    ax[3].plot(np.arange(R + 1), beta_hist.mean(axis=0), c='purple',
               label=r'Mean $\beta$')
    ax[3].plot(np.arange(R), act_hist.mean(axis=0), c='teal',
               label=r'Mean action')
    ax[3].legend(loc='best', fontsize='small')
    ax[3].set(xlim=[0, R], xlabel=r'Round $r$',
              ylabel=r'Boldness / Action')

    fig.savefig(osp.join('..', 'figs', f'coadapt_trial_{rule}.pdf'))


def plot_sweep(N, R, pi, alpha, eps, rule, seed):
    """
    Plots the results of a co-adaptation sweep experiment showing the
    authority's final average parameter values and costs per (mean desired
    dissent, mean boldness) pair.
    ...
    fig.savefig(osp.join('..', 'figs',
                         f'coadapt_sweep_N{N}_R{R}_{pi}_{rule}_S{seed}.png'))


def plot_suppression_times(N, R, pi, alpha, rule, seed, window=500,
                           threshold=0.25, xmax=None):
    """
    ...
    fig.savefig(osp.join('..', 'figs',
                         f'coadapt_suppression_N{N}_R{R}_{pi}_{rule}_S{seed}.pdf'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rule', type=str, default='none',
                        choices=['b2sim', 'b2a', 'd2d', 'd2a', 'a2a',
                                 'd2sim', 'd2max', 'b2b', 'none'],
                        help='Population adaptation rule')
    ...
    if args.sweep:
        ...
        plot_sweep(N=args.num_ind, R=args.rounds, pi=args.pi, alpha=args.alpha,
                   eps=args.epsilon, rule=args.rule, seed=args.seed)
        plot_suppression_times(N=args.num_ind, R=args.rounds, pi=args.pi,
                               alpha=args.alpha, rule=args.rule,
                               seed=args.seed)
    else:
        (taus, psis, nus), pol_costs, pun_costs, deltas, betas, \
            delta_hist, beta_hist, act_hist = \
            coadapt_trial(G=G, N=args.num_ind, R=args.rounds,
